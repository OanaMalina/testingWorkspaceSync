Action()
{
	...
	mqtt_publish(client, "users/qatest2/datapoints", lr_eval_string("Message from ssl publisher {vuid} : {payload}"), MQTT_AUTO, MQTT_QOS1, MQTT_NORETAIN);

	lr_think_time(1);
	
	mqtt_publish(client, "users/qatest2/datapoints", lr_eval_string("Message from ssl publisher {vuid} : {payload}"), MQTT_AUTO, MQTT_QOS0, MQTT_NORETAIN);
